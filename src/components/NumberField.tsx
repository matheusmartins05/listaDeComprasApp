...
export default function NumberField({children, borderRadius, valorDigitado, valorDigitadoState, typeInput} : InputProps) {
  ...
  return (
    <div className="flex gap-1 flex-col ">
      ...
      <input value={valorDigitadoState} onChange={(event) => aoDigitarValores(event)} className={`bg-transparent border border-[#252529] ${borderRadius} h-10 outline-[#A881E6] focus:outline indent-2`} type={typeInput}/>
    </div>
  );
}